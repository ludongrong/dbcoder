...
public class ${className}DaoImpl extends JdbcDaoSupport implements I${className}Dao 
{
    ...
    {
    	final String SQL = "DELETE FROM ${table.sqlName} WHERE ${table.idColumn.sqlName} = ?";
    ...
    {
    	final String SQL = "UPDATE ${table.sqlName} SET <#list table.columns as column>${column.sqlName} = ?<#if column_has_next>,</#if> </#list> WHERE ${table.idColumn.sqlName} = ?";
    	return this.getJdbcTemplate().update(SQL,<#list table.columns as column>${column.columnNameLower}<#if column_has_next>,</#if></#list>);
	}
	...
	{
		final String SQL = "SELECT <#list table.columns as column>${column.sqlName}<#if column_has_next>,</#if></#list> FROM ${table.sqlName} WHERE ${table.idColumn.sqlName} = ?";
		try
		{
