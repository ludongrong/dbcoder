...
public class ${className}Controller
{
	...
	@ResponseBody
	public ${className}Dto create(SessionContext ctxt, HttpServletRequest request,
	...
	public ${className}Dto find(SessionContext ctxt, @RequestBody ${className}Dto ${classNameLower}Dto)
	{
		...
		List<${className}> ${classNameLower}List = this.${classNameLower}Svc.find(${classNameLower}Dto.get${className}Dyna());
	...
	@ResponseBody
	public ${className}Dto findPage(SessionContext ctxt,
			@RequestBody ${className}Dto ${classNameLower}Dto)
	{
		if (ObjectUtil.isNull(${classNameLower}Dto))
		{
		...
		if (ObjectUtil.isNull(${classNameLower}Dto.getPageNum())
				|| CoreUtil.isLessThan(${classNameLower}Dto.getPageNum(), 1))
		{
			throw new CodeException("-1", "PageNum input error");
		}

		// Page Size must more than 1
		if (ObjectUtil.isNull(${classNameLower}Dto.getPageSize())
				|| CoreUtil.isLessThan(${classNameLower}Dto.getPageSize(), 1))
		{
			throw new CodeException("-1", "PageSize input error");
		}
		
		// between time
		if (CollectionUtil.isNotEmpty(${classNameLower}Dyna.getCreateTimeBetween()))
		{
		...
		List<${className}> ${classNameLower}List = this.${classNameLower}Svc.find(${classNameLower}Dto.getPageNum(),
				${classNameLower}Dto.getPageSize(), ${classNameLower}Dto.get${className}Dyna());
		${classNameLower}Dto.set${className}s(${classNameLower}List);
		
		${classNameLower}Dto.setPage(
				this.${classNameLower}Svc.count(new HashMap<>()),
				${classNameLower}Dto.getPageNum(),
				${classNameLower}Dto.getPageSize());
