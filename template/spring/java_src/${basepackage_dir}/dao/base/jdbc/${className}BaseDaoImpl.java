...
public class ${className}BaseDaoImpl extends BaseDaoImpl implements ${className}BaseDao 
{
	...
	private static final Fragment ANALYSIS_FRAGMENT_S = new Fragment()
	{
		@Override
	...
	private final String CREATE = "INSERT INTO ${table.sqlName} (<#list table.columns as column> ${column.sqlName}<#if column_has_next>,</#if></#list>) VALUES (<#list table.columns as column>?<#if column_has_next>,</#if></#list>)";
	
	private final String DESTROY = "DELETE FROM ${table.sqlName} WHERE ${table.idColumn.sqlName} = ?";
	
	private final String UPDATE = "UPDATE ${table.sqlName} SET <#list table.columns as column>${column.sqlName} = ?<#if column_has_next>,</#if> </#list> WHERE ${table.idColumn.sqlName} = ?";
		    
    private final String DETAIL = "SELECT <#list table.columns as column>${column.sqlName}<#if column_has_next>,</#if></#list> FROM ${table.sqlName} WHERE ${table.idColumn.sqlName} = ?";
			
	private final String FIND = "SELECT * FROM (SELECT <#list table.columns as column>${column.sqlName}<#if column_has_next>,</#if> </#list>, ROWNUM RN FROM ${table.sqlName} WHERE ROWNUM < ? {0}) WHERE RN > ?";
	
	private final String COUNT = "SELECT COUNT(ID) FROM ${table.sqlName} {0}";
	...
	ExecuteErrorException
	{
		...
		try
		{
			${classNameLower} = this.getJdbcTemplate().queryForObject(DETAIL, MAPPER_SELF,
					pk);
