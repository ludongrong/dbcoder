...
public class DbmsHandler implements ElementHandler {
    ...
    public void onEnd(ElementPath path) {
        ...
        String name = Optional.ofNullable(element.element("Name")).map(t -> {
            return t.getText();
        }).orElse(null);

        code = toDbType(name.toLowerCase());

        element.detach();
    }

    private String toDbType(String code) {
        String dbType = null;
        if (code != null) {
            ...
            } else if (code.startsWith(JdbcUtils.POSTGRESQL)) {
